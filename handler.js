...
const team_report_template = require('./teamReportTemplate');
export const index = async (event, context) => {
    ...
    const data = {
        ...
        wingSpan: wingSpan,
        reach: reach,
        ten: Number.parseFloat(dash).toFixed(2),
        agility: Number.parseFloat(proAgility).toFixed(2),
        vert: Number.parseFloat(vertical).toFixed(2),
        tenIndex: tenIndex,
...
export const team_report = async (event, context) => {
    let body = null;
    let athleteData = null;
    let averageEpicIndex = null;
    let rosterName = null;
    let averageDashIndex = null;
    let dashRange = null;
    let averageVerticalIndex = null;
    let verticalRange = null;
    let averageProAgilityIndex = null;
    let proAgilityRange = null;

    try {
        body = JSON.parse(Buffer.from(event.body, 'base64').toString());
        athleteData = body.athleteData;
        averageEpicIndex = body.averageEpicIndex;
        rosterName = body.rosterName;
        averageDashIndex = body.averageDashIndex;
        dashRange = body.dashRange;
        averageVerticalIndex = body.averageVerticalIndex;
        verticalRange = body.verticalRange;
        averageProAgilityIndex = body.averageProAgilityIndex;
        proAgilityRange = body.proAgilityRange;
        if (
            athleteData == null ||
            averageEpicIndex == null ||
            rosterName == null ||
            averageDashIndex == null ||
            dashRange == null ||
            averageVerticalIndex == null ||
            verticalRange == null ||
            averageProAgilityIndex == null ||
            proAgilityRange == null
        ) {
            throw 'not enough parameters';
        }
    } catch (err) {
    ...
    athleteData.forEach(function(athlete) {
        athleteNames.push(athlete.name);
        athleteIndexes.push(athlete.index);
    });
    ...
    const data = {
        ...
        dashRangeMin: dashRange.min,
        dashRangeMax: dashRange.max,
        proAgilityRangeMin: proAgilityRange.min,
        proAgilityRangeMax: proAgilityRange.max,
        verticalRangeMin: verticalRange.min,
        verticalRangeMax: verticalRange.max,
        athleteIndexes,
        athleteNames
    };

    const template = handlebars.compile(team_report_template.source, {
        strict: true
    });
    const result = template(data);
